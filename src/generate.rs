...
fn generate_domain<'a>(domain: &Domain,
...
fn generate_type_def(domain: &Domain,
    ...
    let maybe_expr = generate_type_expr_impl(domain_snake_case,
                                             None,
    ...
    domain_index.push_str(&format!("- [`{}`]({}.{}.html)\n",
                                   type_def.name,
                                   category,
                                   type_def_pascal_case));
...
fn generate_type_expr(domain_snake_case: &Ident,
...
fn generate_type_expr_impl(domain_snake_case: &Ident,
                           parent_variant_pascal_case: Option<&Ident>,
                           parent_pascal_case: &Ident,
                           field_name: Option<&String>,
                           deprecation_status: &DeprecationStatus,
    ...
    match *ty {
        ...
        Type::Enum(ref values) => {
            let type_def_pascal_case = combine_parent_field_idents(parent_pascal_case, field_name);
            let note = generate_field_usage_note(domain_snake_case,
                                                 parent_variant_pascal_case,
                                                 parent_pascal_case,
                                                 field_name);
            let meta_attrs =
        ...
        Type::Array { ref item, min_items, max_items } => {
            let item_expr = generate_type_expr(domain_snake_case,
                                               parent_variant_pascal_case,
                                               parent_pascal_case,
                                               field_name,
                                               deprecation_status,
...
fn generate_field_usage_note(domain_snake_case: &Ident,
                             parent_variant_pascal_case: Option<&Ident>,
                             parent_pascal_case: &Ident,
                             field_name: Option<&String>)
                             -> Option<String> {
...
fn generate_method<'a>(domain: &Domain,
                       domain_snake_case: &Ident,
    ...
    domain_index.push_str(&format!("- [`{}`](../enum.{}.html#variant.{})\n",
                                   method_qualified,
                                   category,
                                   variant_pascal_case));
    ...
    let variant_note = generate_method_note(&domain_snake_case,
                                            &method_qualified,
                                            true,
                                            &variant_pascal_case,
                                            ...
                                            category);
    let struct_note = generate_method_note(&domain_snake_case,
                                           &method_qualified,
                                           false,
                                           &variant_pascal_case,
                                           &parameters_struct_pascal_case,
    ...
    let variant_meta_attrs = generate_meta_attrs(&deprecation_status,
                                                 experimental,
                                                 &method.description,
                                                 Some(variant_note));
    ...
    let request_variant_content = if command_refs.is_some() {
        ...
        let fields: Vec<Tokens> = method.parameters
            .iter()
            .map(|field| {
                generate_field(domain_snake_case,
    ...
    if let Some(ref response_struct_pascal_case) = response_struct_pascal_case {
        let fields: Vec<Tokens> = method.returns
            .iter()
            .map(|field| {
                generate_field(domain_snake_case,
                               None,
...
fn generate_field(domain_snake_case: &Ident,
                  parent_variant_pascal_case: Option<&Ident>,
                  parent_pascal_case: &Ident,
                  ...
                  type_defs: &mut Vec<Tokens>)
                  -> Tokens {
    let Field { name: ref field_name, .. } = *field;
    let field_snake_case = snake_case_ident(field_name);
    ...
    let ty = generate_type_expr(domain_snake_case,
                                parent_variant_pascal_case,
                                parent_pascal_case,
                                Some(&field_name),
...
fn generate_meta_attrs(deprecation_status: &DeprecationStatus,
                       experimental: bool,
                       description: &Option<String>,
                       note: Option<String>)
                       -> Tokens {
...
fn resolve_reference(domain_snake_case: &Ident,
                     parent_pascal_case: &Ident,
                     target: &str)
                     -> Tokens {
    lazy_static! {
...
impl DeprecationStatus {
    fn new(deprecated: bool, description: &Option<String>) -> Self {
        ...
        let warning = description.as_ref().and_then(|desc| {
            if desc == "Deprecated." || !DEPRECATION_WARNING_RE.is_match(desc.as_str()) {
                None
            } else {
...
fn generate_method_note(domain_snake_case: &Ident,
