...
fn test_version_info_url_format() {
    ...
    let actual_version_info_url =
        format!(cdp_http_version_info_url_format!(), server_addr = "localhost:9222");
...
fn test_page_list_url_format() {
    ...
    let actual_page_list_url =
        format!(cdp_http_page_list_url_format!(), server_addr = "localhost:9222");
...
fn test_new_page_url_format() {
    ...
    let actual_new_page_url =
        format!(cdp_http_new_page_url_format!(), server_addr = "localhost:9222");
...
fn test_new_page_and_nav_url_format() {
    ...
    let actual_new_page_and_nav_url = format!(
        cdp_http_new_page_and_navigate_url_format!(),
...
fn test_activate_page_url_format() {
    ...
    let actual_activate_page_url = format!(
        cdp_http_activate_page_url_format!(),
        server_addr = "localhost:9222",
        page_id = "0"
    );
...
fn test_command_activate_page() {
    do_test_command("json/activate/0", None, &OwnedCommand::ActivatePage("0".into()));
    do_test_command("json/activate/0/", None, &OwnedCommand::ActivatePage("0/".into()));
    do_test_command("json/activate/0/foo", None, &OwnedCommand::ActivatePage("0/foo".into()));
}
...
fn do_test_command(path: &str, query: Option<&str>, owned_command: &OwnedCommand) {
    ...
    let borrowed_parsed_command =
        Command::parse(path, query).expect("borrowed command parse error");

    let path_with_slash = format!("/{}", path);
    let borrowed_parsed_command_with_slash =
        Command::parse_with_slash(path_with_slash.as_str(), query)
    ...
    let owned_parsed_command =
        OwnedCommand::parse(path, query).expect("owned command parse error");
...
fn test_response_page_list() {
    let json = r#"[
  {
    ...
    let rust = Response::PageList(vec![
        Page {
            id: "0".into(),
            ty: PageType::Tab,
            url: "https://www.mozilla.org".into(),
